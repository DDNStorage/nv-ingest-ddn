...
class Boto3BulkWriter(LocalBulkWriter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            schema, str(local_path), chunk_size, file_type, config, **kwargs
        )
        ...
        self._remote_files = []
        logger.info(
            f"Boto3 S3 buffer writer initialized, target path: {self._remote_path}"
        )
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                ...
                if response.status_code == 200:
                    logger.info(
                        f"Successfully uploaded {file_path} to {remote_file} using presigned URL"
                    )
                    return True
                else:
                    logger.error(
    ...
    def _upload_with_standard_method(self, file_path, remote_file):
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded {file_path} to {remote_file} using put_object"
            )
            return True
        except Exception as e:
        ...
        try:
            ...
            if old_buffer.row_count > 0:
                ...
                for file_path in file_list:
                    ...
                    if self._connect_param.use_presigned:
                        upload_success = self._upload_with_presigned_url(
                            file_path, remote_file
                        )
                    else:
                        upload_success = self._upload_with_standard_method(
                            file_path, remote_file
                        )
                    ...
                    if upload_success:
                        ...
                        raise MilvusException(
                            message=f"Failed to upload {file_path} to S3"
                        )
            ...
            logger.info(
                f"Flush thread finished, name: {threading.current_thread().name}"
            )
