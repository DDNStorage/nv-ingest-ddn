...
def test_valid_log_levels(caplog):
    ...
    for level in LogLevel:
        ...
        assert logger.level == getattr(
            logging, level.value
        ), f"Logger level not set correctly for {level.value}"
...
def test_logger_level_set_correctly(logger):
    ...
    for level in LogLevel:
        ...
        assert logger.level == getattr(
            logging, level.value
        ), "Logger level did not match expected level"
...
def test_logger_raises_for_invalid_level(logger):
    ...
    assert "Invalid log level: NO_LEVEL" in str(
        exc_info.value
    ), "Expected specific error message for invalid log level"
