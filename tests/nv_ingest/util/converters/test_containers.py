...
from nv_ingest.util.converters.containers import (
    merge_dict,
...
def test_merge_simple_dicts():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed for simple dict merge"
...
def test_merge_with_nested_dicts():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed for nested dict merge"
...
def test_merge_with_empty_overrides():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed when overrides are empty"
...
def test_merge_with_empty_defaults():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed when defaults are empty"
...
def test_merge_overrides_none_values():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed when overrides contain None values"
...
def test_merge_does_not_modify_input():
    ...
    assert defaults == {"a": 1, "b": 2} and overrides == {
        "b": 3
    }, "Test failed: function should not modify inputs"
...
def test_merge_with_complex_nested_dicts():
    ...
    expected = {
        "a": 1,
        "b": {"x": {"alpha": "beta", "gamma": "delta"}, "y": 99, "z": 100},
    }
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed for complex nested dict merge"
...
def test_merge_with_lists_should_not_merge():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed: lists should be replaced, not merged"
...
def test_merge_with_differing_types():
    ...
    assert (
        merge_dict(defaults, overrides) == expected
    ), "Test failed for merging differing types"
