...
def test_redis_task_sink_schema_custom_values():
    ...
    """
    custom_redis_client = MessageBrokerClientSchema(
        host="custom_host", port=12345, broker_params={"use_ssl": True}
    )
    schema = MessageBrokerTaskSinkSchema(
        broker_client=custom_redis_client, raise_on_failure=True, progress_engines=10
    )

    assert (
        schema.broker_client.host == "custom_host"
    ), "Custom host value for broker_client should be respected."
    assert (
        schema.broker_client.port == 12345
    ), "Custom port value for broker_client should be respected."
    assert (
