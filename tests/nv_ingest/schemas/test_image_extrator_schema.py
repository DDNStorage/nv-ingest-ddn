...
def test_image_config_schema_invalid_both_services_empty():
    ...
    assert any(
        "Both gRPC and HTTP services cannot be empty" in error["msg"]
        for error in errors
    )
...
def test_image_config_schema_empty_service_strings():
    ...
    assert config.yolox_endpoints == (
        None,
        "http_service_url",
    )  # Cleaned empty strings are None
...
def test_image_extractor_schema_invalid_max_queue_size():
    ...
    errors = exc_info.value.errors()
    assert any(
        error["loc"] == ("max_queue_size",) and error["type"] == "int_parsing"
        for error in errors
    )
...
def test_image_extractor_schema_invalid_n_workers():
    ...
    errors = exc_info.value.errors()
    assert any(
        error["loc"] == ("n_workers",) and error["type"] == "int_parsing"
        for error in errors
    )
