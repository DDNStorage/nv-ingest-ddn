...
def test_source_metadata_schema_defaults():
    config = SourceMetadataSchema(
        source_name="Test Source", source_id="1234", source_type="TestType"
    )
...
def test_content_hierarchy_schema_with_nearby_objects():
    config = ContentHierarchySchema(
        nearby_objects=NearbyObjectsSchema(
            text={"content": ["sample text"]}, images={"content": ["sample image"]}
        )
    )
...
@pytest.mark.parametrize(
    "table_format",
    [
        TableFormatEnum.HTML,
        TableFormatEnum.MARKDOWN,
        TableFormatEnum.LATEX,
        TableFormatEnum.IMAGE,
    ],
)
...
def test_error_metadata_schema_defaults():
    config = ErrorMetadataSchema(
        task="embed", status="error", error_msg="An error occurred."
    )
...
def test_error_metadata_schema_invalid_status():
    with pytest.raises(ValidationError):
        ErrorMetadataSchema(
            task="TaskType1", status="InvalidStatus", error_msg="An error occurred."
        )
