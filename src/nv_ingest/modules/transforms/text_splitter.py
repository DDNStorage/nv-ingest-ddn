...
def _split_into_chunks(text, tokenizer, chunk_size=1024, chunk_overlap=20):
    ...
    chunks = [
        ...
        for i in range(0, len(offsets), chunk_size - chunk_overlap)
    ]
...
TextSplitterLoaderFactory = ModuleLoaderFactory(
    MODULE_NAME, MODULE_NAMESPACE, TextSplitterSchema
)
    ...
    def split_and_forward(message: IngestControlMessage):
        try:
            ...
            bool_index = (df["document_type"] == ContentTypeEnum.TEXT) & (
                pd.json_normalize(df["metadata"])["source_metadata.source_type"].isin(
                    split_source_types
                )
            )
            ...
            if os.path.exists(
                os.path.join(
                    model_predownload_path, "llama-3.2-1b/tokenizer/tokenizer.json"
                )
            ) and (tokenizer is None or tokenizer == "meta-llama/Llama-3.2-1B"):
                tokenizer = os.path.join(
                    model_predownload_path, "llama-3.2-1b/tokenizer/"
                )
            elif os.path.exists(
                os.path.join(
                    model_predownload_path,
                    "e5-large-unsupervised/tokenizer/tokenizer.json",
                )
            ...
            tokenizer_model = AutoTokenizer.from_pretrained(
                tokenizer, token=hf_access_token
            )
            ...
            for _, row in df_filtered.iterrows():
                content = (
                    ...
                    if row["metadata"]["content"] is not None
                    else ""
                )

                chunks = _split_into_chunks(
                    content, tokenizer_model, chunk_size, chunk_overlap
                )
