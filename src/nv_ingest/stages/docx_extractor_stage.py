...
def decode_and_extract(
    ...
    try:
        ...
        if validated_config.docx_extraction_config is not None:
            extract_params["docx_extraction_config"] = (
                validated_config.docx_extraction_config
            )
        ...
        err_msg = (
            f"decode_and_extract: Error loading extractor for file '{source_id}'. "
...
def _process_docx_bytes(
    df, task_props, validated_config: Any, trace_info: Optional[Dict[str, Any]] = None
):
    """
...
def generate_docx_extractor_stage(
    ...
    try:
        ...
        _wrapped_process_fn = functools.partial(
            _process_docx_bytes, validated_config=validated_config
        )
        ...
        err_msg = (
            f"generate_docx_extractor_stage: Error generating document extractor stage. "
            f"Original error: {e}"
        )
        logger.error(err_msg, exc_info=True)
        raise type(e)(err_msg) from e
